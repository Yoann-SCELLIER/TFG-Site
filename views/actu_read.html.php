            ...
            <div class="col p-5">
                <div class="card border-2 m-5">
                    ...
                    <div class="card-body text-center">
                        ...
                        <h5 class="card-title fs-2 m-0"><?= isset($post['title']) ? htmlspecialchars_decode($post['title']) : '' ?></h5>
                    </div>
                    <hr class="border border-2">
                    <div class="card-body">
                        <!-- Affichage du contenu du post avec des sauts de ligne respectés -->
                        <p class="card-text p-3 fs-5 m-0"><?= isset($post['content']) ? nl2br(htmlspecialchars_decode($post['content'])) : '' ?></p>
                        <div class="d-flex justify-content-end">
                            <!-- Affichage de l'auteur du post -->
                            <p class="m-0"><small class="text-body-secondary"><?= isset($post['username']) ? htmlspecialchars_decode($post['username']) : '' ?></small></p>
                        </div>
                    </div>
                    <div class="card-footer text-center"> 
                        <!-- Affichage de la date de création du post -->
                        <p class="m-0">Créé le : <small class="text-body-secondary"><?= isset($post['created_at_fr']) ? htmlspecialchars_decode($post['created_at_fr']) : '' ?></small></p>
                        <!-- Affichage de la date de dernière modification du post (si elle existe) -->
                        <?php if (!empty($post['modif_at_fr'])) : ?>
                            <p class="m-0">Modifié le : <small class="text-body-secondary"><?= isset($post['modif_at_fr']) ? htmlspecialchars_decode($post['modif_at_fr']) : '' ?></small></p>
                        <?php endif; ?>
